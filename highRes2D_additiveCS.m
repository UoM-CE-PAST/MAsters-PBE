...
% - 2023/04/30, MA: small improvements
%
%
...
while t(n)<simulationTime
    if supersaturation(n) >= 1
        ...
        if concentration(n) <= (cmin + 0.75*(cmax-cmin)) && cycleNumber < maxCycleNumber
        ...
        relativeError = 100*abs(concentration(n+1)-concentration(n))/concentration(n+1);
