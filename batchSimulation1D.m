...
operationMode = listdlg("ListString",{'constant temperature (crash cooling)','constant supersaturation','additive crash cooling'},"PromptString",'Please select the required simulation',"SelectionMode","single");
...
%% Constant supersaturation
    case 2
        ...
        legend('Initial PSD (starting from equilibrium)', ...
            'Final PSD (starting from equilibrium)')
    % PSD plots:
%% T. Vetter growth rate/solubility (additive) constant temperature comparison
% this section compares the generic growth rate model to the one used by T.
% Vetter (5)
    case 3
        % prompt user for constant temperature and additive concentration
        prompt = {['Please input the constant temperature at which crystallisation takes place [' char(176) 'C]'],'Please input the concentration of additive used [kg/kg]'};
        dlgtitle = 'Constant temperature';
        definput = {'25','0'};
        answer = inputdlg(prompt,dlgtitle, [1 35],definput);
        constantTemperature = str2double(answer(1));
        additiveConcentration = str2double(answer(2));
        ...
        growthParameters = [0 0.04 0.08; 0.1 9.7 0.15; 4e3 5.3e3 5.5e3; 1.48e4 2.04e4 1.8e4];
        ...
        % method of moments solution:
%         [t_mom, y] = ode23s(@(t_mom, y)mom_additive(t_mom, y, temperatureRamp, k1, k2, k3, p0, p1, p2, p3, p4, ...
%             particleDensity, shapeFactor, operationMode), [0 simulationTime], y0);
%         concentration_mom = y(:,5);
